        ...
        {builds.map((b) => (
          <div key={b.id} className="relative flex flex-col items-start">
            ...
            <button
              ...
              className="absolute -top-2 -right-2 text-red-500 hover:text-red-700 text-xs font-bold rounded-full bg-gray-800 px-1"
            >
              ✕
            </button>
          </div>
      ...
      <div className="p-6 bg-gray-900 border-t border-gray-700">
        ...
        <div className="grid grid-cols-3 gap-2 text-sm">
          {Object.keys(currentBuild.components).map((key) => (
            <div key={key} className="flex flex-row bg-gray-800 p-2 rounded justify-between">
              <div>
                ...
                <div className="text-green-500 inline">{currentBuild.components[key]
                ? "₹"+currentBuild.components[key].price : "₹"+0}</div>
            </div>
          ))}
        </div>
        <h2 className="text-lg font-bold mt-2">Total: <span className="text-green-500">₹{buildTotal}</span></h2>
      </div>
      ...
      {editingBuild && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg w-80 text-white relative">
            ...
            <button
              onClick={() => {
                updateBuildName(editingBuild.id, editingBuild.name);
                updateBuildStatus(editingBuild.id, editingBuild.status);
