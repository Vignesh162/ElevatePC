...
export default function PcBuilderPage() {
    ...
    const [addedCategory, addAddedCategory] = useState([]);
    ...
    const checkComponeant = (components, category) => {
        return Object.values(components).find((c) => c.category.toLowerCase() === category);
    }
                ...
                {builds.map((b) => (
                    <div key={b.id} className="relative flex flex-col items-start">
                        ...
                        <button
                            ...
                            className={`px-3 py-1 rounded ${currentBuildId === b.id
                                ? "bg-blue-600"
                    ...
                    <div className="flex flex-1">
                        ...
                        <div className="w-2/5 flex justify-center p-6">
                            <div className="sticky top-25 w-80 h-[500px] border-4 border-gray-700 rounded-lg bg-gray-900 ">
                                {/* CPU */}
                                <div
                                    className={`absolute top-20 left-30 w-16 h-16 border-2 flex items-center justify-center text-xs text-gray-400 transition ${hoverCategory === "cpu"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"cpu") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    CPU
                                </div>
                                ...
                                {/* GPU */}
                                <div
                                    className={`absolute top-56 left-11 w-54 h-16 border-2 flex items-center justify-center text-xs text-gray-400 transition ${hoverCategory === "gpu"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"gpu") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    GPU
                                </div>
                                ...
                                {/* Motherboard */}
                                <div
                                    className={`absolute top-12 left-8 w-60 h-64 border-2 flex items-center justify-center text-xs text-gray-400 transition ${hoverCategory === "motherboard"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"motherboard") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    Motherboard
                                </div>
                                ...
                                {/* RAM */}
                                <div
                                    className={`absolute top-20 right-14 w-12 h-32 border-2 flex flex-col justify-center items-center text-xs text-gray-400 transition ${hoverCategory === "ram"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"ram") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    RAM
                                </div>
                                ...
                                {/* Storage */}
                                <div
                                    className={`absolute bottom-25 right-6 w-30 h-15 border-2 flex items-center justify-center text-xs text-gray-400 transition ${hoverCategory === "storage"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"storage") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    Storage
                                </div>
                                ...
                                {/* PSU */}
                                <div
                                    className={`absolute bottom-4 left-4 w-40 h-16 border-2 flex items-center justify-center text-xs text-gray-400 transition ${hoverCategory === "psu"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"psu") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    PSU
                                </div>
                                ...
                                {/* CASE */}
                                <div
                                    className={`absolute inset-0 rounded-lg transition ${hoverCategory === "case"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"case") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                />

                                {/* Cooler */}
                                <div
                                    className={`absolute top-22 left-14 w-12 h-12 border-2 rounded-full flex items-center justify-center text-xs text-gray-400 transition ${hoverCategory === "cooler"
                                            ? "border-yellow-400 shadow-lg shadow-yellow-500"
                                            : checkComponeant(currentBuild.components,"cooler") ? " border-blue-700 shadow-md shadow-blue-700" : "border-dashed border-gray-700"
                                        }`}
                                >
                                    Cooler
                                </div>
                        ...
                        <div className="w-3/5 p-6 overflow-y-auto">
                            <div className="w-full mb-4 rounded-lg border border-gray-700 bg-black p-4 space-y-3">
                                <h2 className="text-xl font-bold text-white">
                                    Build Compatibility
                                </h2>

                                <p className="text-sm font-medium text-gray-300">
                                    {currentBuild.compatibility.status === "ok"
                                    ? "✅ Build is compatible"
                                    : currentBuild.compatibility.status === "blocked"
                                    ? "❌ Build is incompatible. Build blocked"
                                    : "⚠️ Build has a few issues"}
                                </p>

                                {/* Warnings */}
                                {currentBuild.compatibility?.warnings?.length > 0 && (
                                    <div className="space-y-1 rounded-md bg-yellow-900/20 border border-yellow-600 p-3">
                                    <h3 className="text-sm font-semibold text-yellow-400">
                                        Warnings
                                    </h3>
                                    {currentBuild.compatibility.warnings.map((warning, idx) => (
                                        <p
                                        key={idx}
                                ...
                                {/* Blockers */}
                                {currentBuild.compatibility?.blockers?.length > 0 && (
                                    <div className="space-y-1 rounded-md bg-red-900/20 border border-red-600 p-3">
                                    <h3 className="text-sm font-semibold text-red-400">
                                        Blockers
                                    </h3>
                                    {currentBuild.compatibility.blockers.map((blocker, idx) => (
                                        <p
                                        key={idx}
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    "Storage", "PSU", "Cooler", "Case", "Monitor", "Gaming Peripherals"
                                ].map((category) => {
                                    // Check if components is an array or object
                                    let component;
                                    if (Array.isArray(Object.values(currentBuild.components))) {
                                        component = Object.values(currentBuild.components).find(comp => comp.category.trim().toLowerCase() === category.trim().toLowerCase());
                                    } else {
